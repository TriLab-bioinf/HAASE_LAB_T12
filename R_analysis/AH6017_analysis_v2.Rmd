...
# Sort results by Log2FC, add gene symbol and save results to a txt file
res.kpt185_dmso.cyto.name.ashr.logfc_sorted <- sort_and_write_res_table(res.kpt185_dmso.cyto.name.ashr, "DE_dmsoNuc_vs_dmsoCyto_ashr")

res.kpt185_dmso.nuc.name.ashr.logfc_sorted <- sort_and_write_res_table(res.kpt185_dmso.nuc.name.ashr, "DE_kpt185Nuc_vs_kpt185Cyto_ashr")
...
for (i in names(DE_results)){
  ...
  logfc_by_exon_count.p <- ggviolin(data = as.data.frame(de_results), x = "exon_range", y = "log2FoldChange", ggtheme = theme_minimal()) #+ ylim(-0.5,0.5)
  logfc_by_exon_count.p <- ggpar(p = logfc_by_exon_count.p, title = i)
  
  ggsave2(filename = paste0(i,"_logFC_by_exon_counts.pdf"), plot = logfc_by_exon_count.p, path = "./Plots")
  
  print(logfc_by_exon_count.p)
}
```
